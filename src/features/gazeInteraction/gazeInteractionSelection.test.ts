...
function processFrame(
  ...
  return processGazeFrameForSelection(gazeFrame, synthetic, 'DWELL', dwell, mouth);
}
...
describe('GazeInteractionProvider 프레임 처리 (registry + hit-test + Step 11 selection 재사용)', () => {
  it('10. DWELL — 큰 target 위에서 threshold(1.2초) 경과 후 정확히 한 번 select된다', () => {
    ...
    processFrame([CARD], frame({ now: BASE }), dwell, mouth);
    const beforeThreshold = processFrame([CARD], frame({ now: BASE + 1199 }), dwell, mouth);
    expect(beforeThreshold.selectedKeyId).toBeNull();

    const atThreshold = processFrame([CARD], frame({ now: BASE + 1200 }), dwell, mouth);
    expect(atThreshold.selectedKeyId).toBe('main-memo');

    // cooldown 중이라 곧바로 같은 위치를 봐도 재선택되지 않는다(더블 액션 방지, §36).
    const rightAfter = processFrame([CARD], frame({ now: BASE + 1210 }), dwell, mouth);
  ...
  it('9. target 이탈 — dwell 진행 중 커서가 rect 밖으로 나가면 progress가 리셋된다', () => {
    ...
    processFrame([CARD], frame({ now: BASE, cursorCssPx: { x: 250, y: 250 } }), dwell, mouth);
    const midway = processFrame([CARD], frame({ now: BASE + 600, cursorCssPx: { x: 250, y: 250 } }), dwell, mouth);
    expect(midway.progress).toBeGreaterThan(0);

    // rect 밖으로 이탈
    const left = processFrame([CARD], frame({ now: BASE + 650, cursorCssPx: { x: 900, y: 900 } }), dwell, mouth);
  ...
  it('8. tracking invalid(hasSignal=false)면 selection이 발생하지 않고 controller가 리셋된다', () => {
    ...
    processFrame([CARD], frame({ now: BASE }), dwell, mouth);
    processFrame([CARD], frame({ now: BASE + 600 }), dwell, mouth);
    ...
    processFrame([CARD], frame({ now: BASE + 800 }), dwell, mouth);
    const resumed = processFrame([CARD], frame({ now: BASE + 900 }), dwell, mouth);
  ...
  it('11. MOUTH — 기존 MouthController 계약(gaze-lock + 입벌림 hold)대로 큰 target도 select된다', () => {
    ...
    function runMouth(gazeFrame: GazeFrame) {
      ...
      const synthetic = toSyntheticSelectionTargets(hit, gazeFrame.cursorCssPx);
      return processGazeFrameForSelection(gazeFrame, synthetic, 'MOUTH', dwell, mouth);
    }
  ...
  it('15. registry가 비면(예: route 이동으로 페이지 target이 전부 unregister) 이전 dwell 진행이 남지 않고 즉시 idle이 된다', () => {
    ...
    processFrame(registry.values(), frame({ now: BASE }), dwell, mouth);
    const midway = processFrame(registry.values(), frame({ now: BASE + 600 }), dwell, mouth);
    expect(midway.progress).toBeGreaterThan(0);

    // MainPage unmount 시뮬레이션 — 모든 target unregister.
    registry.unregister('main-memo');

    const afterUnregister = processFrame(registry.values(), frame({ now: BASE + 650 }), dwell, mouth);
